import Link from "next/link";
import Image from "next/image";
import { ArrowLeft, FileText, Plus } from "lucide-react";

export default function DevisNotFound() {
  return (
    <div className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden px-4 py-12 text-slate-950 dark:text-white">
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-slate-50 via-white to-violet-50 dark:from-slate-950 dark:via-[#0c0a1d] dark:to-violet-950/30" />
      <div className="pointer-events-none absolute inset-x-0 top-0 h-96 bg-[radial-gradient(ellipse_at_top,rgba(124,58,237,0.18),rgba(251,146,60,0.08),transparent_62%)] dark:bg-[radial-gradient(ellipse_at_top,rgba(124,58,237,0.25),rgba(251,146,60,0.1),transparent_60%)]" />

      <div className="relative mx-auto w-full max-w-md">
        <Link
          href="/"
          className="mx-auto mb-8 flex w-fit items-center gap-3 transition hover:opacity-80"
          aria-label="Retour à l'accueil Zolio"
        >
          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/80 ring-1 ring-white/50 dark:bg-white/8 dark:ring-white/10">
            <Image src="/logo.png" alt="Zolio" width={28} height={28} className="h-7 w-auto object-contain" priority />
          </div>
          <span className="text-sm font-semibold tracking-[0.22em] text-violet-600 dark:text-violet-200">ZOLIO</span>
        </Link>

        <div className="rounded-3xl border border-white/60 bg-white/80 p-8 text-center shadow-[0_28px_70px_-36px_rgba(15,23,42,0.35)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/70 sm:p-10">
          <div className="mx-auto mb-5 inline-flex h-16 w-16 items-center justify-center rounded-3xl bg-rose-100 ring-4 ring-rose-50 dark:bg-rose-500/15 dark:ring-rose-500/5">
            <FileText size={30} className="text-rose-600 dark:text-rose-300" />
          </div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-950 dark:text-white">Devis introuvable</h1>
          <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
            Ce devis n&apos;existe pas ou a été supprimé.
          </p>
          <div className="mt-6 flex flex-col gap-2.5 sm:flex-row sm:justify-center">
            <Link
              href="/devis"
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-500 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-violet-500/20 transition hover:opacity-90"
            >
              <ArrowLeft size={16} />
              Retour aux devis
            </Link>
            <Link
              href="/nouveau-devis"
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200/80 bg-white/80 px-5 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-violet-300 hover:text-violet-700 dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:hover:border-violet-400/20"
            >
              <Plus size={16} />
              Nouveau devis
            </Link>
          </div>
        </div>
      </div>
    </div>
  );
}
