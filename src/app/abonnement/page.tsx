...
export default function AbonnementPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen pb-8 font-sans max-w-md md:max-w-3xl lg:max-w-5xl mx-auto w-full bg-white dark:bg-gray-800 dark:bg-slate-900 sm:shadow-xl sm:my-4 sm:rounded-[3rem] overflow-hidden relative selection:bg-fuchsia-200">
      ...
      <header className="relative z-10 flex items-center gap-4 p-6 pt-12 sm:pt-10">
        <Link href="/">
          <motion.div whileTap={{ scale: 0.9 }} className="w-10 h-10 bg-white dark:bg-gray-800/10 dark:bg-slate-900/10 backdrop-blur-md border border-white/20 rounded-full flex items-center justify-center text-white">
            <ArrowLeft size={20} />
          </motion.div>
        </Link>
      </header>
      ...
      <main className="relative z-10 flex-1 px-6 flex flex-col mt-4">
        ...
        {/* Pricing Card */}
        <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="bg-white dark:bg-gray-800 dark:bg-slate-900 rounded-3xl p-6 shadow-2xl shadow-fuchsia-900/20 border border-slate-100 dark:border-slate-800 relative">
          
          {/* Toggle */}
          <div className="flex bg-slate-100 dark:bg-slate-800 rounded-xl p-1 mb-6 relative">
            <div className={`absolute inset-y-1 w-[calc(50%-4px)] bg-white dark:bg-gray-800 dark:bg-slate-900 rounded-lg shadow-sm transition-all duration-300 ${isAnnual ? 'left-[calc(50%+2px)]' : 'left-1'}`} />
