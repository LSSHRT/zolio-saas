...
const NAV_ITEMS: Omit<CmdKItem, "action">[] = [
  { id: "new-devis", label: "Nouveau devis", shortcut: "N", icon: Plus, section: "Actions rapides" },
  { id: "new-facture", label: "Nouvelle facture", icon: FileText, section: "Actions rapides" },
  { id: "new-depense", label: "Nouvelle dépense", icon: Receipt, section: "Actions rapides" },
  { id: "devis", label: "Devis", icon: FileCheck2, section: "Navigation" },
  { id: "factures", label: "Factures", icon: FileText, section: "Navigation" },
  { id: "clients", label: "Clients", icon: Users, section: "Navigation" },
  { id: "depenses", label: "Dépenses", icon: Receipt, section: "Navigation" },
  { id: "dashboard", label: "Tableau de bord", icon: BarChart3, section: "Navigation" },
  { id: "parametres", label: "Paramètres", shortcut: "S", icon: Settings, section: "Navigation" },
];
...
export default function CommandPalette({ open, onClose }: CommandPaletteProps) {
  ...
  const allFlat = useMemo(() => grouped.flatMap((g) => g.items), [grouped]);
  ...
  // Global shortcut: ⌘K closes when palette is open
  useEffect(() => {
    ...
    <>
      ...
      >
        <div className="flex items-center gap-3 border-b border-slate-200 px-5 py-4 dark:border-white/10">
          <Search size={20} className="shrink-0 text-slate-400" aria-hidden />
          <input
            ref={inputRef}
            type="text"
            value={query}
            ...
            aria-label="Rechercher dans la palette"
          />
          <button
            type="button"
            onClick={onClose}
            aria-label="Fermer la palette de commandes"
            className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/40 dark:hover:bg-white/10"
          >
            <X size={16} aria-hidden />
          </button>
        </div>

        <div className="max-h-[26rem] overflow-y-auto overscroll-contain py-2" role="listbox">
          {grouped.length === 0 ? (
            <div className="px-5 py-10 text-center">
              <p className="text-sm font-semibold text-slate-900 dark:text-white">Aucun résultat</p>
