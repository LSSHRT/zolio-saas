...
export function DashboardRecentQuotes({ items, onSelectDevis }: { items: QuoteListItem[], onSelectDevis?: (numero: string) => void }) {
  ...
  return (
    <>
      ...
      {/* Desktop View (Table) */}
      <div className="hidden lg:block overflow-hidden rounded-2xl border border-slate-200/70 bg-white/75 dark:border-white/8 dark:bg-white/4">
  ...
  return (
    <>
      ...
      <div className="space-y-3 lg:hidden">
        {items.slice(0, 3).map((item) => (
          <Link href={`/devis/${item.numero}`} key={item.numero}>
            <div className="rounded-2xl border border-rose-200/70 bg-rose-50/80 p-4 dark:border-rose-400/12 dark:bg-rose-500/8">
      ...
      {/* Desktop View */}
      <div className="hidden lg:block overflow-hidden rounded-2xl border border-rose-200/70 bg-white/75 dark:border-rose-400/12 dark:bg-white/4">
        <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
